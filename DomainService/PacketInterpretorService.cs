...
public interface IPacketInterpretorService
{
    Task ProcessPlanes(long now);
}

public partial class PacketInterpretorService(IPlaneRepository planeRepository, IClock clock) : IPacketInterpretorService
{

    public async Task ProcessPlanes(long now)
    {
        var nextIcao = "";

        //Grab the next plane
        while (!string.IsNullOrEmpty(nextIcao = await planeRepository.GetNextIcao(now)))
        {
            await HandleMessagesForIcao(nextIcao, now);
        }
    }

    private async Task HandleMessagesForIcao(string icao, long time)
    {
        ...
        {
            //Interpret packet
            await ApplyFrame( currentRecord, packet);
        ...
        await planeRepository.MarkIcaoMomentAsComplete(icao, time);
    }

    private async Task ApplyFrame(Plane plane, string frame)
    {
        ...
        {
            var altitude = decodeAC13Field(frame);
            if (altitude >= 0)
            {
    ...
    public async Task RecordPacket(string serialNumber, string frame)
    {
        ...
        try
        {
            ...
            if (messageType == 0 || messageType == 4 || messageType == 16 || messageType == 20)
            {
                ...
                if (altitude >= 0)
                {
//                    await planeRepository.SetAltitude(icao, altitude);
                }
            }
            ...
            switch (messageType)
            {
                ...
                case 0 or 4 or 16 or 20:
                    // planeRepository.SetAltitude() decodeAC13Field(frame);
                    break;
                case 17:
            ...
            if (messageType == 17)
            {
                var tc = (byte)(byte.Parse(frame.Substring(4 * 2, 2), System.Globalization.NumberStyles.HexNumber) >> 3);
                if (tc >= 9 && tc <= 18)
                {
                    var cpr = (byte)(byte.Parse(frame.Substring(6 * 2 + 1, 1), System.Globalization.NumberStyles.HexNumber) & 0x4);
                    // Console.WriteLine($"{icao} {cpr} {now}:{now.Millisecond}::  {messageType} : {tc} {cpr} @ ");
                }
            }

            if (string.IsNullOrEmpty(icao))
            {
                // Console.WriteLine($"{frame} lacked icao");
                return;
            }
            ...
            // await planeRepository.RecordPacket(frame, icao, (long)Math.Floor((clock.GetUtcNow() - DateTime.UnixEpoch).TotalSeconds));
